...
namespace VErpApi.Controllers.Stock.Inventory
{
    ...
    public class InventoryController : VErpBaseController
    {
        ...
        public async Task<PageData<ProductPackageOutputModel>> GetPackageListForExport([FromQuery] string keyword, [FromQuery] bool? isTwoUnit, [FromQuery] IList<long> packageIds, [FromQuery] IList<int> stockIds, [FromQuery] int page, [FromQuery] int size)
        {
            return await _inventoryService.GetProductPackageListForExport(keyword, isTwoUnit, packageIds, stockIds, page, size);
        }
