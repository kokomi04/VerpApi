...
namespace VErp.Services.PurchaseOrder.Service.Input.Implement
{
    public class InputConfigService : IInputConfigService
    {
        ...
        public InputConfigService(PurchaseOrderDBContext purchaseOrderDBContext
        ...
        {
            var inputType = await _purchaseOrderDBContext.InputType
        ...
        public async Task<PageData<InputTypeModel>> GetInputTypes(string keyword, int page, int size)
        {
            ...
            var query = _purchaseOrderDBContext.InputType.AsQueryable();
        ...
        public async Task<int> AddInputType(InputTypeModel data)
        {
            ...
            var existedInput = await _purchaseOrderDBContext.InputType
            ...
            try
            {
                ...
                await _purchaseOrderDBContext.InputType.AddAsync(inputType);
        ...
        public string GetStringClone(string source, int suffix = 0)
        {
            ...
            if (_purchaseOrderDBContext.InputType.Any(i => i.InputTypeCode == code))
            {
                suffix++;
        ...
        {
            using var @lock = await DistributedLockFactory.GetLockAsync(DistributedLockFactory.GetLockInputTypeKey(0));
            var sourceInput = await _purchaseOrderDBContext.InputType
            ...
            try
            {
                ...
                await _purchaseOrderDBContext.InputType.AddAsync(cloneType);
                await _purchaseOrderDBContext.SaveChangesAsync();
                ...
                foreach (var area in sourceInput.InputArea)
                {
                    ...
                    await _purchaseOrderDBContext.InputArea.AddAsync(cloneArea);
                    await _purchaseOrderDBContext.SaveChangesAsync();
                    ...
                    foreach (var field in sourceInput.InputAreaField.Where(f => f.InputAreaId == area.InputAreaId).ToList())
                    {
                        ...
                        await _purchaseOrderDBContext.InputAreaField.AddAsync(cloneField);
                    }
                }
                await _purchaseOrderDBContext.SaveChangesAsync();
                trans.Commit();
        ...
        public async Task<bool> UpdateInputType(int inputTypeId, InputTypeModel data)
        {
            ...
            var inputType = await _purchaseOrderDBContext.InputType.FirstOrDefaultAsync(i => i.InputTypeId == inputTypeId);
            if (inputType == null)
            {
                throw new BadRequestException(InputErrorCode.InputTypeNotFound);
            }
        ...
        public async Task<bool> DeleteInputType(int inputTypeId)
        {
            ...
            await _purchaseOrderDBContext.ExecuteStoreProcedure("asp_InputType_Delete", new[] {
        ...
        {
            return await _purchaseOrderDBContext.InputTypeView.Where(v => v.InputTypeId == inputTypeId).ProjectTo<InputTypeViewModelList>(_mapper.ConfigurationProvider).ToListAsync();
        }

        public async Task<InputTypeBasicOutput> GetInputTypeBasicInfo(int inputTypeId)
        {
            var inputTypeInfo = await _purchaseOrderDBContext.InputType.AsNoTracking().Where(t => t.InputTypeId == inputTypeId).ProjectTo<InputTypeBasicOutput>(_mapper.ConfigurationProvider).FirstOrDefaultAsync();

            inputTypeInfo.Areas = await _purchaseOrderDBContext.InputArea.AsNoTracking().Where(a => a.InputTypeId == inputTypeId).OrderBy(a => a.SortOrder).ProjectTo<InputAreaBasicOutput>(_mapper.ConfigurationProvider).ToListAsync();

            var fields = await (
                from af in _purchaseOrderDBContext.InputAreaField
                join f in _purchaseOrderDBContext.InputField on af.InputFieldId equals f.InputFieldId
            ...
            var views = await _purchaseOrderDBContext.InputTypeView.AsNoTracking().Where(t => t.InputTypeId == inputTypeId).OrderByDescending(v => v.IsDefault).ProjectTo<InputTypeViewModelList>(_mapper.ConfigurationProvider).ToListAsync();
        ...
        {
            var info = await _purchaseOrderDBContext.InputTypeView.AsNoTracking().Where(t => t.InputTypeId == inputTypeId && t.InputTypeViewId == inputTypeViewId).ProjectTo<InputTypeViewModel>(_mapper.ConfigurationProvider).FirstOrDefaultAsync();
            ...
            var fields = await _purchaseOrderDBContext.InputTypeViewField.AsNoTracking()
        ...
        public async Task<int> InputTypeViewCreate(int inputTypeId, InputTypeViewModel model)
        {
            ...
            try
            {
                var inputTypeInfo = await _purchaseOrderDBContext.InputType.FirstOrDefaultAsync(t => t.InputTypeId == inputTypeId);
                ...
                await _purchaseOrderDBContext.InputTypeView.AddAsync(info);
                await _purchaseOrderDBContext.SaveChangesAsync();
        ...
        public async Task<bool> InputTypeViewUpdate(int inputTypeViewId, InputTypeViewModel model)
        {
            ...
            try
            {
                ...
                var info = await _purchaseOrderDBContext.InputTypeView.FirstOrDefaultAsync(v => v.InputTypeViewId == inputTypeViewId);

                if (info == null) throw new BadRequestException(GeneralCode.ItemNotFound, "View không tồn tại");

                var inputTypeInfo = await _purchaseOrderDBContext.InputType.FirstOrDefaultAsync(t => t.InputTypeId == info.InputTypeId);
                if (inputTypeInfo == null) throw new BadRequestException(GeneralCode.ItemNotFound, "Không tìm thấy loại chứng từ");


                _mapper.Map(model, info);

                var oldFields = await _purchaseOrderDBContext.InputTypeViewField.Where(f => f.InputTypeViewId == inputTypeViewId).ToListAsync();

                _purchaseOrderDBContext.InputTypeViewField.RemoveRange(oldFields);
        ...
        public async Task<int> InputTypeGroupCreate(InputTypeGroupModel model)
        {
            ...
            await _purchaseOrderDBContext.InputTypeGroup.AddAsync(info);
            await _purchaseOrderDBContext.SaveChangesAsync();
        ...
        {
            var info = await _purchaseOrderDBContext.InputTypeGroup.FirstOrDefaultAsync(g => g.InputTypeGroupId == inputTypeGroupId);
        ...
        {
            return await _purchaseOrderDBContext.InputTypeGroup.ProjectTo<InputTypeGroupList>(_mapper.ConfigurationProvider).OrderBy(g => g.SortOrder).ToListAsync();
        }
        ...
        private async Task InputTypeViewFieldAddRange(int inputTypeViewId, IList<InputTypeViewFieldModel> fieldModels)
        {
            ...
            await _purchaseOrderDBContext.InputTypeViewField.AddRangeAsync(fields);
        ...
        {
            var inputArea = await _purchaseOrderDBContext.InputArea
        ...
        {
            keyword = (keyword ?? "").Trim();
            var query = _purchaseOrderDBContext.InputArea.Where(a => a.InputTypeId == inputTypeId).AsQueryable();
            if (!string.IsNullOrEmpty(keyword))
            {
        ...
        {
            using var @lock = await DistributedLockFactory.GetLockAsync(DistributedLockFactory.GetLockInputTypeKey(inputTypeId));
            var existedInput = await _purchaseOrderDBContext.InputArea
            ...
            if (data.IsMultiRow && _purchaseOrderDBContext.InputArea.Any(a => a.InputTypeId == inputTypeId && a.IsMultiRow))
            {
            ...
            try
            {
                ...
                await _purchaseOrderDBContext.InputArea.AddAsync(inputArea);
                await _purchaseOrderDBContext.SaveChangesAsync();
        ...
        {
            using var @lock = await DistributedLockFactory.GetLockAsync(DistributedLockFactory.GetLockInputTypeKey(inputTypeId));
            var inputArea = await _purchaseOrderDBContext.InputArea.FirstOrDefaultAsync(a => a.InputTypeId == inputTypeId && a.InputAreaId == inputAreaId);
            if (inputArea == null)
            {
            ...
            if (data.IsMultiRow && _purchaseOrderDBContext.InputArea.Any(a => a.InputTypeId == inputTypeId && a.InputAreaId != inputAreaId && a.IsMultiRow))
            {
                throw new BadRequestException(InputErrorCode.MultiRowAreaAlreadyExisted);
            }
        ...
        public async Task<bool> DeleteInputArea(int inputTypeId, int inputAreaId)
        {
            ...
            await _purchaseOrderDBContext.ExecuteStoreProcedure("asp_InputArea_Delete", new[] {
                    new SqlParameter("@InputTypeId",inputTypeId ),
        ...
        {
            keyword = (keyword ?? "").Trim();
            var query = _purchaseOrderDBContext.InputAreaField
        ...
        {
            keyword = (keyword ?? "").Trim();
            var query = _purchaseOrderDBContext.InputField
                .AsQueryable();
            if (!string.IsNullOrEmpty(keyword))
            {
        ...
        {
            var inputAreaField = await _purchaseOrderDBContext.InputAreaField
        ...
        private void ValidateInputField(InputFieldInputModel data, InputField inputField = null, int? inputFieldId = null)
        {
            if (inputFieldId.HasValue && inputFieldId.Value > 0)
            {
                ...
                if (_purchaseOrderDBContext.InputField.Any(f => f.InputFieldId != inputFieldId.Value && f.FieldName == data.FieldName))
                {
        ...
        {
            var inputTypeInfo = await _purchaseOrderDBContext.InputType.AsNoTracking()
            ...
            var inputAreas = await _purchaseOrderDBContext.InputArea.Where(a => a.InputTypeId == inputTypeId).AsNoTracking().ToListAsync();
            ...
            var curFields = _purchaseOrderDBContext.InputAreaField
            ...
            try
            {
                ...
                foreach (var deleteField in deleteFields)
                {
                    ...
                    await _purchaseOrderDBContext.ExecuteStoreProcedure("asp_InputType_Clear_FieldData", new[] {
                        new SqlParameter("@InputTypeId",inputTypeId ),
                ...
                foreach (var field in fields)
                {
                    ...
                    if (curField == null)
                    {
                        ...
                        await _purchaseOrderDBContext.InputAreaField.AddAsync(curField);
        ...
        public async Task<int> AddInputField(InputFieldInputModel data)
        {
            ...
            try
            {
                ...
                await _purchaseOrderDBContext.InputField.AddAsync(inputField);
                await _purchaseOrderDBContext.SaveChangesAsync();

                if (inputField.FormTypeId != (int)EnumFormType.ViewOnly)
                {
                    await _purchaseOrderDBContext.AddColumn(INPUTVALUEROW_TABLE, data.FieldName, data.DataTypeId, data.DataSize, data.DecimalPlace, data.DefaultValue, true);
                }
        ...
        {
            var inputField = await _purchaseOrderDBContext.InputField.FirstOrDefaultAsync(f => f.InputFieldId == inputFieldId);
            ...
            try
            {
                if (inputField.FormTypeId != (int)EnumFormType.ViewOnly)
                {
                    ...
                    {
                        await _purchaseOrderDBContext.RenameColumn(INPUTVALUEROW_TABLE, inputField.FieldName, data.FieldName);
                    }
                    await _purchaseOrderDBContext.UpdateColumn(INPUTVALUEROW_TABLE, data.FieldName, data.DataTypeId, data.DataSize, data.DecimalPlace, data.DefaultValue, true);
                }
        ...
        public async Task<bool> DeleteInputField(int inputFieldId)
        {
            ...
            // Check used
            bool isUsed = _purchaseOrderDBContext.InputAreaField.Any(af => af.InputFieldId == inputFieldId);
            if (isUsed)
            {
            ...
            try
            {
                ...
                if (inputField.FormTypeId != (int)EnumFormType.ViewOnly)
                {
                    await _purchaseOrderDBContext.DropColumn(INPUTVALUEROW_TABLE, inputField.FieldName);
                }
                await UpdateInputValueView();
        ...
        {
            await _purchaseOrderDBContext.ExecuteStoreProcedure("asp_InputValueRow_UpdateView", Array.Empty<SqlParameter>());
        }
    }
}
