...
DECLARE @HrTypeModuleId INT = 111002
DECLARE @HrTypeObjectTypeId INT  = 1037
;WITH tmp AS (
	SELECT 
	t.HrTypeId	
	...
	(
		SELECT t.HrTypeId
		FROM OrganizationDB.dbo.HrType t 
		WHERE t.IsDeleted = 0		
	) t
)
...
SELECT  r.RoleId,
	...
	AND NOT EXISTS
	(
		SELECT 0 FROM dbo.RolePermission p WHERE p.RoleId = r.RoleId AND p.ModuleId = @HrTypeModuleId AND p.ObjectTypeId = @HrTypeObjectTypeId AND p.ObjectId = m.HrTypeId
	)