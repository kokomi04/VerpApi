...
Select @FieldCheckStepString = concat(@FieldCheckStepString+',',f.VoucherFieldId) from PurchaseOrderDB.dbo.VoucherField f where (f.ObjectApprovalStepTypeId & @CheckStepTypeId) = @CheckStepTypeId;
SET @FieldCheckStepString = concat('[',@FieldCheckStepString,']');

--Lấy danh sách fieldId cho buoc duyet
Select @FieldApprovalStepString = concat(@FieldApprovalStepString+',',f.VoucherFieldId) from PurchaseOrderDB.dbo.VoucherField f where (f.ObjectApprovalStepTypeId & @ApprovalStepTypeId) = @ApprovalStepTypeId;
